...
send_start()  { send_marker_raw "start,sigmark,${1}"; } # $1 = elapsed seconds (integer/string tag)
send_stop()   { send_marker_raw "stop,sigmark,${1}"; }  # $1 = elapsed seconds (integer/string tag)

# ------------------------------------------
# Stress sequence (only ONE 90% block)
# ------------------------------------------
STRESS_CMDS=(
  "stress-ng -l 0  --cpu 1 -t 20s"
...
do_stress_next() {
  if (( NEXT_STRESS_IDX < ${#STRESS_CMDS[@]} )); then
    ...
    ${STRESS_CMDS[$NEXT_STRESS_IDX]}
    ((NEXT_STRESS_IDX++))
  else
    echo "NOTE: No more stress steps left; skipping."
  fi
}

# ------------------------------------------
# Your original timeline with loops unchanged,
# except send_tick -> do_stress_next
# ------------------------------------------
